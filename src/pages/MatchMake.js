...
export const PlayerMatchmake = () => {
    ...
    const handle_add = (event) => {
        ...
        if (player.support === "" || player.damage === "" || player.tank === "" || player.battleTag === "") {
    ...
    const generate_player_table = (players, index_key) => {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            result.push(<TableCell className={styles.text_color} key={index_key + player.name + i}>{player.name}</TableCell>);
        }
        return result
    }
            ...
            return <TableRow key={index}>
                {
                    [
                        ...
                        <TableCell className={styles.text_color} key={"avg_rank" + index}>{player_group.avg_rank}</TableCell>
                    ]
                }
            </TableRow>;
        })
    ...
    const generate_table = () => {
        return <Table>
            <TableHead>
                <TableRow>
                    <TableCell className={styles.text_color} key="tank1">Tank 1</TableCell>
                    <TableCell className={styles.text_color} key="tank2">Tank 2</TableCell>
                    <TableCell className={styles.text_color} key="dps1">Damage 1</TableCell>
                    <TableCell className={styles.text_color} key="dps2">Damage 2</TableCell>
                    <TableCell className={styles.text_color} key="sp1">Support 1</TableCell>
                    <TableCell className={styles.text_color} key="sp2">Support 2</TableCell>
                    <TableCell className={styles.text_color} key="avg">Average Rating</TableCell>
                </TableRow>
            </TableHead>
            <TableBody>
                {generate_rows()}
    ...
    const generate_form_table = () => {
        ...
        for (let i = 0; i < curr.length; i++) {
            ...
            result.push(
                <TableRow key={i}>
                    <TableCell className={styles.text_color} key={"name" + i}>{player.name}</TableCell>
                    <TableCell className={styles.text_color} key={"tank_rating" + i}>{player.tank}</TableCell>
                    <TableCell className={styles.text_color} key={"dps_rating" + i}>{player.damage}</TableCell>
                    <TableCell className={styles.text_color} key={"sp_rating" + i}>{player.support}</TableCell>
    ...
    return (
        <div className={styles.App}>
            <div className={styles.App_header}>
                ...
                <h1>Match Page</h1>
                <p>
                    Have a bunch of friends you want to play but don't know who can play with each other?
                    <br />
                ...
                <form>
                    ...
                    <FormControlLabel control={<Checkbox
                        onChange={(event) => setState({ ...state, max_players: event.target.checked })}
                    />}
